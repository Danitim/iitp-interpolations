...
def bilinear_interpolation(image, x_new, y_new):
    ...
    x_grid, y_grid = np.meshgrid(np.linspace(0, width - 1, x_new), np.linspace(0, height - 1, y_new))
    ...
    for c in range(image.shape[2] if image.ndim == 3 else 1):
        ...
        top = Ia + dx * (Ic - Ia)
        bottom = Ib + dx * (Id - Ib)
        resized_image[..., c] = (top + dy * (bottom - top)) if image.ndim == 3 else (top + dy * (bottom - top))
